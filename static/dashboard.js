...
async function carregarMetricas() {
    try {
        ...
        // Usa os dados já injetados pelo Flask no HTML (evita chamada extra à API)
        if (window.METRICAS_INICIAIS) {
            dados = window.METRICAS_INICIAIS;
            console.log('✅ Métricas carregadas via Flask template.');
        } else {
        ...
        // Atualizar valores na tela com os nomes exatos retornados pela API/Flask
        if (document.getElementById('co2-value')) {
            document.getElementById('co2-value').textContent = formatter.format(dados.co2_total_g || 0);
        }
        if (document.getElementById('time-value')) {
            document.getElementById('time-value').textContent = formatter.format(dados.tempo_total_min || 0);
        }
        if (document.getElementById('fuel-value')) {
            document.getElementById('fuel-value').textContent = formatter.format(dados.combustivel_total_ml || 0);
        }
        if (document.getElementById('tree-value')) {
            const arvores = dados.equivalente_arvores || 0;
...
function alternarAba(abaNome) {
    ...
    } catch(e) { /* chamada programática sem evento de clique */ }
}
